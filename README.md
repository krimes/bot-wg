...
| `WEB_PASSWORD` | общий пароль веб-выдачи первого WG. Пусто — сайт выключен |
| `WEB_SECRET` | (опц.) секрет cookie-сессии; иначе считается из пароля |
| `WEB_HOST` / `WEB_PORT` | где слушать сайт, по умолчанию `0.0.0.0:8080` |

## Веб-выдача первого WG

Сайт для **первичного** конфига AmneziaWG. Пароль (`WEB_PASSWORD`) раздаёте
только админам; дальше человек выбирает себя в списке (ФИ, ник, Telegram ID)
и жмёт **Сгенерировать**.

- в списке только `ADMIN_IDS` и включённые пользователи из базы бота;
- повторная генерация для того же Telegram ID — ошибка;
- Happ с сайта не выдаётся.

После логина: `http://IP_СЕРВЕРА:8080`. Не публикуйте порт в интернет без
HTTPS/reverse proxy, если сервер не за VPN.
